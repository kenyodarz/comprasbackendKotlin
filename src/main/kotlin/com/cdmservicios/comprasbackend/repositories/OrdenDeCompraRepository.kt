...
interface OrdenDeCompraRepository : JpaRepository<OrdenDeCompra, Int> {
    ...
    @Query(
        "select oc.idordendecompra, oc.numerodeorden, oc.fechadeorden, oc.condestinoa, oc.contacto" +
                ", oc.concargoa, oc.transportador, oc.numerodeguia, oc.formadepago, oc.idusuario" +
                ", oc.observaciones, oc.idrequisicion, oc.fechaderegistro, oc.fechadeactualizado" +
                ", oc.iva, oc.exentodeiva, oc.centrodecostos, oc.valoriva, oc.anulada, pv.*" +
                ...
                "left join ( select rp.idpedido, sum(rp.cantidadrecibida) as recibidos " +
                "from recepciondepedidos rp group by rp.idpedido order by 1) as r on r.idpedido = ped.idpedido " +
